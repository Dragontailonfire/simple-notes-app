...
export const notes = pgTable("notes", {
    ...
    pgPolicy("Individuals can delete their own notes.", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.uid() = user_id)` }),
    pgPolicy("Individuals can update their own notes.", { as: "permissive", for: "update", to: ["public"], using: sql`(auth.uid() = user_id)` }),
    pgPolicy("Individuals can view their own notes.", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
]);
